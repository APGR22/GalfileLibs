...
namespace galfile::filesystem
{
    class Filesystem
    {
        ...
        protected:
            std::shared_ptr<folder::Folder> _go_to_folder(
                const path::Path &path
            ) const
            {
                std::shared_ptr<folder::Folder> curdir_ptr;
                if (path.is_absolute())
                {
                ...
                {
                    curdir_ptr = this->__curdir_ptr.lock();
                }

                const auto &path_parts = path.parts();
                std::shared_ptr<folder::Folder> temp_curdir_ptr;
                ...
                )
                {
                    ...
                    temp_curdir_ptr = curdir_ptr->get_folder(path_name);
                    if (!temp_curdir_ptr) return {};
        ...
        public:
            ...
            std::shared_ptr<folder::Folder> mkdirs(const path::Path &path)
            {
                std::shared_ptr<folder::Folder> curdir_ptr;
                if (path.is_absolute())
                {
            ...
            std::shared_ptr<file::File> mkfile(
            ...
            )
            {
                ...
                if (!folder_ptr) return {};

                if (folder_ptr->is_file_exists(filename)) return {};

                return folder_ptr->append_file(
                    file::create_new(
                        physical_filepath,
                        filename,
                        true
            ...
            {
                auto folder_ptr = this->_go_to_folder(path);
                if (!folder_ptr) return false;

                if (folder_ptr.get() == this->__root.get())
                {
                    this->__root->clear();
                ...
                auto parent_folder_ptr = folder_ptr->get_parent();
                if (!parent_folder_ptr) return false;

                this->__curdir_ptr = parent_folder_ptr;
                this->__curdir_path = path.parent();

                return parent_folder_ptr->remove_folder(
                    folder_ptr->get_name()
                );
            }
            ...
            bool rmfile(const path::Path &filepath)
            {
                ...
                if (!folder_ptr->is_file_exists(filename)) return false;

                return folder_ptr->remove_file(filename);
            }

            std::shared_ptr<folder::Folder> cpdir(
            ...
            )
            {
                ...
                if (!src_folder_ptr) return {};
                if (!dst_parent_folder_ptr) return {};

                return dst_parent_folder_ptr->append_folder(
            ...
            std::shared_ptr<file::File> cpfile(
            ...
            )
            {
                ...
                if (!src_parent_filepath_ptr) return {};
                if (!dst_parent_filepath_ptr) return {};

                auto src_file_ptr = src_parent_filepath_ptr->get_file(filename);
                if (!src_file_ptr) return {};

                return dst_parent_filepath_ptr->append_file(
            ...
            std::shared_ptr<folder::Folder> mvdir(
                const path::Path &src_path,
                const path::Path &dst_parent_path
            )
            {
                ...
                if (!src_folder_ptr) return {};
                if (!src_parent_folder_ptr) return {};
                if (!dst_parent_folder_ptr) return {};

                auto mv_ret = dst_parent_folder_ptr->append_folder(
                    src_folder_ptr
                );
                if (!mv_ret) return {};

                auto rm_ret = src_parent_folder_ptr->remove_folder(
                    src_folder_ptr->get_name()
                );
                if (!rm_ret) return {};
            ...
            std::shared_ptr<file::File> mvfile(
                const path::Path &src_filepath,
                const path::Path &dst_parent_filepath
            )
            {
                ...
                auto mv_ret = dst_parent_filepath_ptr->append_file(
                    src_file_ptr
                );
                if (!mv_ret) return {};

                auto rm_ret = src_parent_filepath_ptr->remove_file(filename);
                if (!rm_ret) return {};
            ...
            bool isfile(const path::Path &filepath) const
            {
                ...
                if (!directory_ptr) return false;

                return directory_ptr->is_file_exists(filename);
            }
